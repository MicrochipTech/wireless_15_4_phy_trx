...
void trx_reg_write(uint8_t addr, uint8_t value)
{
    ...
    SERCOM4_SPI_Write(&addr, 1);
    
    /* Write the byte in the transceiver data register */
    while (SERCOM4_SPI_IsBusy()){}
    SERCOM4_SPI_Write(&value, 1);
...
uint8_t trx_reg_read(uint8_t addr)
{
   ...
   /* Read the byte from the transceiver data register */
    while (SERCOM4_SPI_IsBusy()){}
    SERCOM4_SPI_Read(&register_value, 1);
...
void trx_frame_write(uint8_t* buf, uint8_t length)
{
    ...
    SERCOM4_SPI_Write(&temp, 1);
    while (SERCOM4_SPI_IsBusy()){}
    /* Write into the buffer */
    SERCOM4_SPI_Write(buf, length);
